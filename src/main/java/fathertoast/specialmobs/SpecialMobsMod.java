...
public class SpecialMobsMod {
    ...
    public static final String VERSION = "1.0.3-ruukas";

    /*
     * Primary features: - Re-implement ranged capabilities for (cave) spiders?
     *
     * Utility features: - Bestiary
     *
     * Notes: - Texture update packets are untested
     */

    /**
     * Handles operations when different behaviors are needed between the client and
     * server sides.
     */
    ...
    public void preInit( FMLPreInitializationEvent event ) {
        ...
        if (isInDev) {
            /*
             * This line updates the loot table asset files. Assets are copied to a
             * temporary directory on run, so the game needs to be started a second time
    ...
    /**
     * Registers the entities in this mod and their resources (e.g., their loot
     * tables).
     */
    ...
    private void registerSpecialVariant( int id, EnumMobFamily.Species variant ) {
        ...
        ResourceLocation lootTable = LootTableList.register( new ResourceLocation( SpecialMobsMod.LOOT_TABLE_PATH + variant.family.name.toLowerCase() + "/" + variant.name.toLowerCase() ) );
        try {
        ...
        if (Config.get().GENERAL.SPAWN_EGGS) {
            EntityRegistry.registerEgg( new ResourceLocation( SpecialMobsMod.NAMESPACE + variant.unlocalizedName ), variant.family.eggBaseColor, variant.bestiaryInfo.eggSpotsColor );
        }
    }
    ...
    private void registerEntity( int id, String unlocalizedName, Class<? extends Entity> entityClass, int trackingRange, int trackingFrequency, boolean trackVelocity ) {
        EntityRegistry.registerModEntity( new ResourceLocation( SpecialMobsMod.NAMESPACE + unlocalizedName ), entityClass, unlocalizedName, id, this, trackingRange, trackingFrequency, trackVelocity );
    }
}
